<template>
    <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex items-center justify-end gap-3">
            <SecondaryButton type="button" @click="$inertia.visit(route('oraciones.index'))">
                Cancelar
            </SecondaryButton>
...
const form = useForm({
    titulo: props.oracion?.titulo ?? '',
    descripcion: props.oracion?.descripcion ?? '',
    tipo: props.oracion?.tipo ?? 'Librillo',
    imagen_id: props.oracion?.imagen_id ?? null,
    imagen: null,
    precio: props.oracion?.precio ?? 0,
});

const tipos = ['Librillo', 'Audio'];

const submitLabel = props.mode === 'edit' ? 'Actualizar' : 'Guardar';

const imagenPreviewUrl = props.oracion?.imagen?.ruta ? `/storage/${props.oracion.imagen.ruta}` : '';
...
const submit = () => {
    if (props.mode === 'edit' && props.oracion) {
        form.transform((data) => ({ ...data, _method: 'put' }))
            .post(route('oraciones.update', props.oracion.id), { forceFormData: true });
        return;
    }

    form.post(route('oraciones.store'));
};
</script>
