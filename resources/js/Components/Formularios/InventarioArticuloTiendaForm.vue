<template>
    <form @submit.prevent="submit" class="space-y-6">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                <InputLabel for="articulo_tienda_id" value="Artículo" />
                <select
                    id="articulo_tienda_id"
                    v-model="form.articulo_tienda_id"
                ...
                <InputError class="mt-2" :message="form.errors.articulo_tienda_id" />
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <SecondaryButton type="button" @click="$inertia.visit(route('inventario-articulos-tienda.index'))">
                Cancelar
            </SecondaryButton>
...
const form = useForm({
    articulo_tienda_id: props.inventario?.articulo_tienda_id ?? null,
...
const submit = () => {
    if (props.mode === 'edit' && props.inventario) {
        form.put(route('inventario-articulos-tienda.update', props.inventario.id));
        return;
    }

    form.post(route('inventario-articulos-tienda.store'));
};
</script>
