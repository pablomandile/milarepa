...
const inventariosFiltradosMobile = computed(() => {
    ...
    return props.inventarios.filter((i) => {
        const campos = [i.articulo_tienda?.titulo, i.articulo_tienda?.categoria?.nombre, String(i.cantidad ?? '')];
...
const eliminar = (id) => {
    ...
    router.delete(route('inventario-articulos-tienda.destroy', id));
};

const portadaUrl = (inventario) => {
    if (inventario?.articulo_tienda?.imagen?.ruta) {
        return `/storage/${inventario.articulo_tienda.imagen.ruta}`;
    }
...
<template>
    <Head title="Inventario Artículos de Tienda" />

    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Inventario Artículos de Tienda</h1>
        </template>
                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 max-w-6xl mx-auto">
                    <div class="flex justify-between flex-wrap gap-2">
                        <div class="flex flex-wrap gap-2">
                            <Link :href="route('inventario-articulos-tienda.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">
                                NUEVO REGISTRO
                            </Link>
                        </div>
                    </div>
                        ...
                        >
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="h-20 w-14 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        <img
                                            ...
                                            :alt="`Imagen de ${inventario.articulo_tienda?.titulo || 'artículo'}`"
                                            class="max-h-full max-w-full object-contain"
                                        />
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-base font-semibold text-gray-800 dark:text-gray-100 break-words">{{ inventario.articulo_tienda?.titulo ?? 'Sin artículo' }}</p>
                                        <p v-if="inventario.articulo_tienda?.categoria?.nombre" class="text-sm text-gray-600 dark:text-gray-400">{{ inventario.articulo_tienda.categoria.nombre }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3">
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    <Link
                                        :href="route('inventario-articulos-tienda.edit', inventario.id)"
                    ...
                    <div class="mt-4 hidden sm:block">
                        <DataTable
                            ...
                            :globalFilterFields="['articulo_tienda.titulo', 'articulo_tienda.categoria.nombre']"
                            stripedRows
                            paginator
                            :rows="10"
                        ...
                        <Column header="Imagen" style="width: 110px">
                            <template #body="{ data }">
                                <div class="h-16 w-12 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                                    <img
                                        :src="portadaUrl(data)"
                                        :alt="`Imagen de ${data.articulo_tienda?.titulo || 'artículo'}`"
                                        class="max-h-full max-w-full object-contain"
                                    />
                                </div>
                            </template>
                        ...
                        <Column header="Artículo">
                            <template #body="{ data }">
                                {{ data.articulo_tienda?.titulo ?? 'Sin artículo' }}
                            </template>
                        </Column>
                        <Column header="Categoría">
                            <template #body="{ data }">
                                {{ data.articulo_tienda?.categoria?.nombre ?? '-' }}
                            </template>
                        </Column>
                        ...
                        <Column header="Acciones" style="width: 180px">
                            <template #body="{ data }">
                                <div class="flex justify-center items-center space-x-4">
                                    <Link
                                        :href="route('inventario-articulos-tienda.edit', data.id)"
