...
const inventariosFiltradosMobile = computed(() => {
    ...
    return props.inventarios.filter((i) => {
        const campos = [i.otro?.titulo, i.otro?.tipo, String(i.cantidad ?? '')];
...
const eliminar = (id) => {
    ...
    router.delete(route('inventario-otros.destroy', id));
};

const portadaUrl = (inventario) => {
    if (inventario?.otro?.imagen?.ruta) {
        return `/storage/${inventario.otro.imagen.ruta}`;
    }
...
<template>
    <Head title="Inventario Otros" />

    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Inventario Otros</h1>
        </template>
                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 max-w-6xl mx-auto">
                    <div class="flex justify-between flex-wrap gap-2">
                        <div class="flex flex-wrap gap-2">
                            <Link :href="route('inventario-otros.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">
                                NUEVO REGISTRO
                            </Link>
                        </div>
                    </div>
                        ...
                        >
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="h-20 w-14 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        <img
                                            ...
                                            :alt="`Portada de ${inventario.otro?.titulo || 'artículo'}`"
                                            class="max-h-full max-w-full object-contain"
                                        />
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-base font-semibold text-gray-800 dark:text-gray-100 break-words">{{ inventario.otro?.titulo ?? 'Sin artículo' }}</p>
                                        <p v-if="inventario.otro?.tipo" class="text-sm text-gray-600 dark:text-gray-400">{{ inventario.otro.tipo }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3">
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    <Link
                                        :href="route('inventario-otros.edit', inventario.id)"
                    ...
                    <div class="mt-4 hidden sm:block">
                        <DataTable
                            ...
                            :globalFilterFields="['otro.titulo', 'otro.tipo']"
                            stripedRows
                            paginator
                            :rows="10"
                        ...
                        <Column header="Portada" style="width: 110px">
                            <template #body="{ data }">
                                <div class="h-16 w-12 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                                    <img
                                        :src="portadaUrl(data)"
                                        :alt="`Portada de ${data.otro?.titulo || 'artículo'}`"
                                        class="max-h-full max-w-full object-contain"
                                    />
                                </div>
                            </template>
                        ...
                        <Column header="Artículo">
                            <template #body="{ data }">
                                {{ data.otro?.titulo ?? 'Sin artículo' }}
                            </template>
                        </Column>
                        <Column field="otro.tipo" header="Tipo" />
                        <Column field="cantidad" header="Cantidad total" />
                        <Column header="Acciones" style="width: 180px">
                            <template #body="{ data }">
                                <div class="flex justify-center items-center space-x-4">
                                    <Link
                                        :href="route('inventario-otros.edit', data.id)"
