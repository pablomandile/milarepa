...
const nombreEditado = ref(props.esquemaPrecio.nombre);
const guardandoNombre = ref(false);

function iniciarEdicionNombre() {
  nombreEditado.value = props.esquemaPrecio.nombre;
...
function guardarNombre() {
  ...
  if (!nuevo) return;
  if (nuevo === props.esquemaPrecio.nombre) {
    editandoNombre.value = false;
    return;
  }
  guardandoNombre.value = true;
  router.put(route('esquemaprecios.update', props.esquemaPrecio.id), { nombre: nuevo }, {
    preserveScroll: true,
    onSuccess: () => {
      editandoNombre.value = false;
    },
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-2 mb-4">
                    <template v-if="!editandoNombre">
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ esquemaPrecio.nombre }}</h2>
                        <button
                            type="button"
