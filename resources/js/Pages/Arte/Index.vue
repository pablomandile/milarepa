...
const arteFiltradoMobile = computed(() => {
    const term = (filters.value.global.value || '').toString().trim().toLowerCase();
    if (!term) return props.arte;
    return props.arte.filter((a) => {
        const campos = [a.titulo, a.tipo];
        return campos.some((v) => String(v ?? '').toLowerCase().includes(term));
    });
});

const eliminar = (id) => {
    if (!window.confirm('Desea eliminar este artículo de arte?')) {
        return;
    }

    router.delete(route('arte.destroy', id));
};
...
const portadaUrl = (item) => {
    if (item?.imagen?.ruta) {
        return `/storage/${item.imagen.ruta}`;
    }
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Arte</h1>
        </template>
                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 max-w-6xl mx-auto">
                    <div class="flex justify-between flex-wrap gap-2">
                        <div class="flex gap-2">
                            <Link :href="route('arte.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">
                                NUEVO ARTÍCULO
                            </Link>
                        </div>
                    </div>
                    ...
                    <!-- Buscador móvil -->
                    <div v-if="arte.length > 0" class="sm:hidden mt-4">
                    ...
                    <!-- Tarjetas móvil -->
                    <div v-if="arteFiltradoMobile.length > 0" class="space-y-4 sm:hidden mt-4">
                        <div
                            v-for="item in arteFiltradoMobile"
                            :key="item.id"
                            ...
                            <div class="space-y-3 p-4">
                                <div class="flex items-start gap-3">
                                    <div class="h-20 w-14 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        <img
                                            :src="portadaUrl(item)"
                                            :alt="`Portada de ${item.titulo || 'arte'}`"
                                            class="max-h-full max-w-full object-contain"
                                        />
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-base font-semibold text-gray-800 dark:text-gray-100 break-words">{{ item.titulo || '-' }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center justify-between gap-3 text-sm">
                                        ...
                                        <span class="text-right">{{ item.tipo || '-' }}</span>
                                    </div>
                                    <div class="flex items-center justify-between gap-3 text-sm">
                                        <span class="text-gray-500">Precio</span>
                                        <span class="text-right font-semibold">{{ formatPrice(item.precio) }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div v-else-if="arte.length > 0" class="sm:hidden mt-4 text-center py-8 text-gray-500 dark:text-gray-400">
                        ...
                        <Column header="Portada" style="width: 110px">
                            <template #body="{ data }">
                                <div class="h-16 w-12 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                                    <img
                                        :src="portadaUrl(data)"
                                        :alt="`Portada de ${data.titulo || 'arte'}`"
                                        class="max-h-full max-w-full object-contain"
                                    />
                                </div>
                            </template>
                        ...
                        <Column header="Acciones" style="width: 180px">
                            <template #body="{ data }">
                                <div class="flex justify-center items-center space-x-4">
                                    <Link
                                        :href="route('arte.edit', data.id)"
                                        v-tooltip="'Editar arte'"
