...
const otrosFiltradosMobile = computed(() => {
    const term = (filters.value.global.value || '').toString().trim().toLowerCase();
    if (!term) return props.otros;
    return props.otros.filter((o) => {
...
const eliminar = (id) => {
    ...
    router.delete(route('otros.destroy', id));
};
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Otros</h1>
        </template>
                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 max-w-6xl mx-auto">
                    <div class="flex justify-between flex-wrap gap-2">
                        <div class="flex gap-2">
                            <Link :href="route('otros.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">
                                NUEVO ARTÍCULO
                            </Link>
                        </div>
                    </div>
                    ...
                    <!-- Buscador móvil -->
                    <div v-if="otros.length > 0" class="sm:hidden mt-4">
                    ...
                    <!-- Tarjetas móvil -->
                    <div v-if="otrosFiltradosMobile.length > 0" class="space-y-4 sm:hidden mt-4">
                        <div
                            v-for="item in otrosFiltradosMobile"
                            :key="item.id"
                            ...
                            <div class="space-y-3 p-4">
                                <div class="flex items-start gap-3">
                                    <div class="h-20 w-14 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        <img
                                            :src="portadaUrl(item)"
                                            :alt="`Portada de ${item.titulo || 'artículo'}`"
                    ...
                    <div v-else-if="otros.length > 0" class="sm:hidden mt-4 text-center py-8 text-gray-500 dark:text-gray-400">
                        ...
                        <Column header="Portada" style="width: 110px">
                            <template #body="{ data }">
                                <div class="h-16 w-12 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                                    <img
                                        :src="portadaUrl(data)"
                                        :alt="`Portada de ${data.titulo || 'artículo'}`"
                                        class="max-h-full max-w-full object-contain"
                                    />
                                </div>
                            </template>
